...
**Goal:** Impedir en MySQL la eliminación de colonia, familia, edificio y vivienda, aun cuando no tengan relaciones hijas, y permitir borrar calle y persona cuando no tengan dependencias.

**Architecture:** Una migración SQL instala cuatro disparadores `BEFORE DELETE` que emiten SQLSTATE `45000`. Las claves foráneas de vivienda y persona protegen calle y persona. `DataAccessException` convierte el error del disparador en un mensaje legible para JavaFX.
...
Crear cuatro `CREATE TRIGGER ... BEFORE DELETE` que emitan:
...
Ejecutar el recurso SQL en el contenedor local de MySQL y consultar `information_schema.TRIGGERS` para verificar las cuatro reglas instaladas.
